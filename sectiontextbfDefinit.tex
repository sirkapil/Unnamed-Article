...
(a \ , \ x) \rightarrow ax
\end{equation*}
\marginnote[-2em]{$\forall \ a \in \ R$ and $x$ $\in \ M$}
satisfying following properties :

\begin{align}
(a+b) x =& ax + bx \\ a(x+y) =& ax + ay \\ (ab)x =& a(bx) \\ 1x =& x
\end{align}
\marginnote[-7em]{\large{$\forall$ $ a\ , \ b \in R$ \newline $x \ , y \ \in M$}}
...
\begin{description}
		\item[(i) \centering{Preserves Addition}]

\begin{equation*}
		(a+b)f = (a)f +(b)f
		
\end{equation*}
		\item[(ii) \centering{Preservers Multiplication} ]

\begin{equation*}
		(ab)f = (a)f . (b)f
		
\end{equation*}
		\item[(iii) \centering{ Maps identity to identity}]
...
\begin{align*}
   ...
   \end{align*}
\marginnote[-5.5em]{using defination of $\phi_a$} \marginnote[-4em]{using \eqref{eq1.9}}
...
\[
\text{for each}\  x \in M \ \text{we have,}
\]
...
\begin{align*}
    (ab)f &= \phi_{ab}\mathnote{

\begin{align*}
 (x)\phi_{ab} &= x(ab)   = (xa)b \\ &= (xa)\phi_b = (x)\phi_a \centerdot\phi_b \\ \therefore \ \phi_{ab} &= \phi_a\centerdot\phi_b \end{align*}
 } \\  &= \phi_a\centerdot\phi_b \\&= (a)f\ (b)f
...
\[
\therefore \ \phi_{1} \ \text{is identity of}End \ M 
\]

\begin{align*}
 (x)\phi_{1} &= x(1) \\&=x \end{align*}
} &= \phi_{1}
...
\begin{thm}
[\textbf{Criterion for Checking Modules} ]
...
\begin{definition}
[\textbf{Cyclic module} ]   \cite{Cohn2005}
...
\end{align*}
    \item[Case-II] Suppose $S$ is non-empty and let    
         ...
         Let $x \in\ S$ then it can be expressed in following form:
          \marginnote[-2.5em]{with $a_1 = 1$ and $x_1 = x $}
           \marginnote[-2em]{$\because x$ was chosen arbitirary.}
         ...
         Let $u\ ,\ v \in P$ . so, we need to show $u + \alpha v \in P$ \marginnote{for any $\alpha\in R$}
          \marginnote[-2em]{$\forall x_i \in S \ \&\ a_i \in R$}
            \marginnote[-2em]{$\forall y_j \in S \ \&\ b_j \in R$}
...
\end{align*}
          ...
          $\therefore \  K$  is closed under scaler multiplication and addition.
            \marginnote[-2.5em]{$\forall \ a_i \in R \ \& \ x_i \in S $}
          so , 
...
\begin{defn}
[\textbf{Generating Set / Set of Generators} ]
...
\begin{defn}
  Let $M$ be a left $R$-module and $\left\lbrace N_\alpha\right\rbrace_{\alpha \in \Omega }$ \marginnote{where $\Omega$ is indexing set. } be family of submodules of $M$ then sum $\sum\limits_{\alpha \in \Omega } N_\alpha $ is defined to be a submodule of $M$ generated by $\bigcup\limits_{\alpha \in \Omega}N_\alpha $   \bigskip
...
\begin{prop}
    Let $M$ be a left $R$-module and $\left\lbrace N_\alpha\right\rbrace_{\alpha \in \Omega }$ \marginnote{where $\Omega$ is indexing set. } be family of submodules of $M$ then sum  \marginnote[-2em]{for each $\alpha \in \Omega$}
